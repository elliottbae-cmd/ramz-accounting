...
for dm_name, group in df.groupby('DM', sort=True):
    ...
    for _, row in group.iterrows():
        ...
        # Color logic: green = <60%, grey = 60–65%, red = >65%
        if pct_used > 0.65:
...
row_num += 1
for hex_c, label in [(OVER_RED, '> 65% of Weekly Goal Used'), (NEUTRAL, '60% – 65% of Weekly Goal Used'), (UNDER_GRN, '< 60% of Weekly Goal Used')]:
